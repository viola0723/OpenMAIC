...
import { ArrowRight, Lock, LoaderCircle, ShieldCheck } from 'lucide-react';

interface SettingsCodeModalProps {
  open: boolean;
  onSuccess: () => void;
  onCancel: () => void;
}

export function SettingsCodeModal({ open, onSuccess, onCancel }: SettingsCodeModalProps) {
  const { t } = useI18n();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/settings-code/verify', {
        method: 'POST',
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          <div className="absolute inset-0 bg-background" onClick={onCancel}>
            <div
              ...
              >
                <div className="relative">
                  <input
                    ref={inputRef}
                    type="password"
                    placeholder={t('settingsCode.placeholder')}
                    value={code}
                    onChange={(e) => {
                ...
                {/* Unlike the site-wide gate this modal is optional chrome,
                    so it offers a way out. */}
                <button
                  type="button"
                  onClick={onCancel}
                  className="w-full text-center text-sm text-muted-foreground/70 transition-colors hover:text-muted-foreground cursor-pointer"
                >
                  {t('settingsCode.cancel')}
                </button>
              </motion.form>
            </div>
          </motion.div>
